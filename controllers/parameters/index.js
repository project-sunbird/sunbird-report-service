...
const debug = require('debug')('parameters:index');
 ...
 */
const populateReportsWithParameters = (reports, user) => _.reduce(reports, (results, report) => {
  ...
  if (isParameterized) {
    if (user) {
      ...
      if (isUserSuperAdmin(user)) {
        ...
        const childReports = _.uniqBy(_.concat(
          _.filter(_.get(report, 'children'), (child) => hash.includes(_.get(child, 'hashed_val'))),
...
const getDataset = async ({ dataSource, user, req }) => {
  ...
  try {
    const dataset = {
      dataset_id: id, isParameterized: false, parameters: null, data: [],
    };
    ...
    return {
      dataset_id: id, data: [], parameters: null, isParameterized: false,
    };
  }
};
...
module.exports = {
  populateReportsWithParameters, getDatasets, reportParameters: parameters, isReportParameterized,
};
