...
const {
  AZURE: {
    account_key, account_name, container_name, sasExpiryTime,
  },
} = envVariables;

const blobService = azure.createBlobService(account_name, account_key);

const getBlobReadStream = ({ filePath }) => blobService.createReadStream(container_name, filePath);

const checkIfBlobExists = ({ container = container_name, filePath }) => new Promise((resolve, reject) => {
...
const getSharedAccessSignature = ({
  container = container_name, filePath, headers = {}, expiryTime = sasExpiryTime,
